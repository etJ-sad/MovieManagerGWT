...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "movies_web" because\n';
  ...
  $wnd.__gwt_activeModules['movies_web'] = {
    'moduleName' : 'movies_web',
    'bindings' : function() {
      return {};
    }
  };
 ...
 * "/movies_web.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['movies_web'].superdevmode = true;

  var expectedSuffix = '/movies_web.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + movies_web + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:movies_web';
  var devModeSessionKey = '__gwtDevModeSession:movies_web';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/movies_web/movies_web.nocache.js';
  ...
  $wnd.__gwt_activeModules['movies_web'].canRedirect = true;
  $wnd.__gwt_activeModules['movies_web'].superdevmode = true;
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/movies_web?user.agent=' + ua + '&_callback=' + callback;
